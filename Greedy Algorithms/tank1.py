...
# Wyznaczamy stacje na których tankujemy tak, żeby łączna liczba tankowań była minimalna.Gdy czołg zatrzymuje się na stacji to tankuje na niej do pełna.
...
def tank(A,L,S):
    ...
    while True:
        ...
        #gdy na obecnym baku jestem w stanie dojechać do celu
        if A - currLocation <= L:
            return cnt
        ...
        #szukam mozliwie najdalszej stacji, która znajduje się w zasięgu baku
        while idx + 1 < len(S) and S[idx+1] <= longestDist: 
            idx += 1 
        
        #jeśli w zasięgu nie ma żadnej stacji
        if (idx == lastStation):
            return None
